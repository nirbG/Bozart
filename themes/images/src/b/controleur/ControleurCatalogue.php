...
class ControleurCatalogue  {
    ...
    function detail($nom,$id){
        ...
        $random=Produit::where('id','like','%'.rand(1, 9).'%')->take(3)->get();
        $bS=$this->bestSeller();
        $req=array("categorie"=>$cat,"sousliste"=>$sl,"prod"=>$prod,"VousAimerai"=>$Va,"random"=>$random,"select"=>$sel,"bestSeller"=>$bS);
        $vue=new VueCatalogue($req);
        $html=$vue->render(5);
        echo $html;
    }
    ...
    function ajProd($id){
        ...
        $dmc = $app->request->post('dmc');
        $s = $app->request->post($dmc);
        $select=Select::where('nom','=',$dmc)->first();
        $option='image';
        if($select!=null) {
            if($select->id!=6) {
                if($select->id==7){
    ...
    function test(){
        ini_set('max_execution_time',-1);
            ...
            foreach ($opts as $opt) {
                echo "$opt->nom<br>";
                $a = explode('-', $opt->nom);
                //SelectProd2opt::where(['idSelect' => $opt->idSelect, 'idprod' => $opt->idprod, 'nom' => $opt->nom])->update(["nom"=>$a[0]]);
