                ...
                return{
                    init : function(){
                        ...
                        $('#CalculerPrix').click(function(e) {
                            ...
                            var prixM=10;
                            var res=(longueur+largeur)*1.20*prixM;
                            console.log(res);
                          $(".price").html("<Strong>Prix : "+res+" €</strong>");
                        });
						/*$("option").click(function (event) {
                			$(".price").html("<Strong>Price : "+$(event.target).attr("prix")+"</strong>");
            			});*/
                    }
                }
            })();
        ...
        $cont=" <div class='affichageoption'>
                ...
                <div class='row'>
					<div class='span9'>								
						...
						<h4 class='title'><span>Nos<strong>Marque :</strong></span></h4>";
        //initialisation du contenu
        $i=1;
                    ...
                    <div class="span3" id="col">
						<div class="block ">
							<ul class="nav nav-list">
								...
								<li class=""><a href="$urlcatalogue">tout</a></li>
    ...
    public function CatalogueByM($chemin){
        ...
        $cont="
                ...
                <div class='row'>
					<div class='span9'>								
						<ul class='thumbnails listing-products' id='catalogue'>
						<h4 class='title'><span>Nos<strong>Produit :</strong></span></h4>";
        //initialisation du contenu
        $i=1;
            ...
            if($trie->nom==$sl->nom) {
                ...
                $cont = $cont . <<<END
                   <li class="span3">
						<div class="product-box">
							$promo											
							<a href="$url"><img alt="" src="$chemin/images/Products/$sl->nom/$item->img"></a><br/>
							<div class='desc'>
							    <div class="nom">
        ...
        return $cont.<<<end
                    ...
                    <div class="span3" id="col">
						<div class="block ">
							<ul class="nav nav-list">
								<li class="nav-header">Trie</li>
								<li class="$tout"><a href="$urlT0">tout</a></li>
								<li class="$pc"><a href="$urlT1">Prix Croisssant</a></li>
								<li class="$pd"><a href="$urlT2">Prix Decroissant</a></li>
    ...
    function detail($chemin){
        ...
        if($sel =! null) {
            ...
            foreach ($sel as $itsel){
                ...
                if($itsel->id!=6){
                        $option = $option . <<<END
                        <p class="radio"><input type="radio" name="dmc" value="$itsel->nom"> $itsel->nom</p>
END;
                        if($itsel->id!=8) {
                            $option = $option . <<<END
							<select class="mySelect" name="$itsel->nom">
END;
...
END;
                        ...
                        $option = $option . <<<END
                        <div class="mySelect">
                         largeur :<input type='number' name='largeur' step="any" value=''><br>
                         longueur :<input type='number' name='longueur' step="any" value=''><br>
                         <a id="CalculerPrix">calculer prix</a>
                        </div>
END;
                    }
                    ...
                    $css="style='width: 95%;'";
                    $palletteBut="<li class=''><a href='#palette'>Palette de couleur</a></li>";
                    $option="";
                    $pallette=<<<end
                    ...
                    foreach ($opt as $color) {
                        $pallette =$pallette.<<<END
                        <tr class="itemNuancier">
				     		<form class="form-inline" id='form1' method='POST' action='$ajout'>
				     		<input type="hidden" name="dmc" value="$itsel->nom">
				     	    <input type="hidden" name="$itsel->nom" value="$color->nom">
				     		<th>
								<img src="$chemin/images/Products/palette/$color->img">
							</th>
							<td>
								<label class="couleurC">
	    							<h4>$color->nom</h4>
	    							<span>Qty:<input type="number" name="quantite" min="1" class="span1 spanPalette" placeholder="1"></label></span>
								</div>
		    					<div class="couleurP">
	    	    	  				<label><h4 class="price palleteprice"><strong>Prix: $color->prix €</strong></h4></label>
			   						<button class="btn btn-inverse" type="submit" style="width: 100%;">Ajouter au panier</button>
	    						</div>
	    					</td>
	    					</form>
						</tr>
            ...
            <section class="main-content">				
				<div class="row">						
					<div class="span9d" $css>
						<div class="row">
							<div class="span4">
								<a href="$chemin/images/Products/all/$prod->img" class="thumbnail" data-fancybox-group="group1" title="$prod->nom"><img alt="" src="$chemin/images/Products/$sl->nom/$prod->img"></a>
							...
							<div class="span5d">
								...
								<h4 class="price"><strong>$prod->prix €</strong></h4>
							</div>
							<div class="span5d">
								<form class="form-inline" id='form1' method='POST' action='$ajout'>
								    $option
						...
						<div class="row">
							...
							<div class="span9d" style="width: 100%;">
								<br>
								<h4 class="title">
									<span class="pull-left"><span class="text"><strong>Vous aimerai</strong> peu etre </span></span>
									<span class="pull-right">
...
END;
            ...
            $cont = $cont . <<<END
							<li class="span3 $bye">
								<div class="product-box">
									$promo
									<p><a href="$urldetail"><img src="$chemin/images/Products/$sl->nom/$item->img" alt="" /></a></p>
									<div class="nom">
									    <img src="$chemin/images/$dispo">
...
END;
    ...
    function prix($item){
        ...
        if($sel->count()==0){
            ...
            $form="<h3>A Partir de</h3><p class='price'>$item->prix €</p>";
        }
        return $form;
    }
            ...
            $cont=$cont.<<<end
                        <li>
							<a href="$urldetail" title="$p->nom">
								<img src="$chemin/images/Products/$sl->nom/$p->img" alt="$p->nom">
							</a>
...
END;
    ...
    function palette($chemin){
        ...
        foreach ($colors as $color) {
            $app=\Slim\Slim::getInstance();
            $ajout = $app->urlFor('ajProd', ['id' => $prod->id]);
            $condi="<select class='mySelectP' name='Palette' size='1' >";
            ...
            $palette =$palette.<<<END
                        <tr class="itemNuancier">
				     		<form class="form-inline" id='form1' method='POST' action='$ajout'>
				     		<input type="hidden" name="dmc" value="Palette">
				     		<th>
								<img src="$chemin/images/Products/palette/$color->img">
							</th>
							<td>
		    					...
		    					<div class="couleurP">
		    						<button class="btn btn-inverse" type="submit" style="float: right;">Ajouter</button>
	    	    	  				...
	    	    	  				<span>Qty:<input type="number" min="1" name="quantite" class="span1 spanPalette" placeholder="1"></span>
			   					</div>
	    					</td>
	    					</form>
						</tr>
END;
    ...
    function imageAdd($chemin){
        ...
        foreach ($images as $image){
            $img.=<<<end
                    <li class="span1">
						<a href="$chemin/images/Products/$sl->nom/$image->nom" class="thumbnail" data-fancybox-group="group1" title="$image->descImg"><img src="$chemin/images/Products/$sl->nom/$image->nom" alt=""></a>
					</li>
end;
        }
        return $img;
